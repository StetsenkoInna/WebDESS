		...
		if ($elem.hasClass('petri-object')) {
			...
			$(document).one('mouseup', function(e) {
				...
				if ($elem.hasClass('petri-object')) {
					...
					var firstObject = currentModel.objects.filter(function(item) { return item.id === firstObjectId; })[0];
					var secondObject = currentModel.objects.filter(function(item) { return item.id === secondObjectId; })[0];
...
function restoreModel(jsonModel) {
	...
	for (var i = 0; i < model.objects.length; i++) {
		...
		var petriNets = netOptions.filter(function(item) { return item.netId === object.netId; });
		if (!petriNets.length) {
			restoringOk = false;
			break;
		}
	...
	$.each(model.objects, function(p, object) {
		object.arcs = model.arcs.filter(function(arcElem) { return arcElem.firstObjectId === object.id || arcElem.secondObjectId === object.id; }).slice();
	});
	return model;
}
...
function deleteArc(id) {
	net = null;
	var theseArcs = currentModel.arcs.filter(function(item) { return item.id === id; });
	if (theseArcs.length) {
		var arcToDelete = theseArcs[0];
		var objects = currentModel.objects.filter(function(item) { return item.id === arcToDelete.firstObjectId || item.id === arcToDelete.secondObjectId; });
		if (objects) {
...
function deleteObject(id) {
	net = null;
	var theseObjects = currentModel.objects.filter(function(item) { return item.id === id; });
...
function getNetById(netId) {
	var netOptions = getNetOptions();
	return restorePetriNet(parsePetriNet(netOptions.filter(function(item) { return item.netId === netId; })[0].net));
}
...
function addMoreSimilarObjects(objectId, number) {
	net = null;
	var initialObject = currentModel.objects.filter(function(item) { return item.id === objectId; })[0];
...
function editArcsForObject(objectId) {
	...
	var object = currentModel.objects.filter(function(item) { return item.id === objectId; })[0];
	var arcs = object.arcs;
	for (var j = 0; j < arcs.length; j++) {
		var arc = arcs[j];
		var newHtml = '';
		if (arc.firstObjectId === objectId) {
			var secondObject = currentModel.objects.filter(function(item) { return item.id === arc.secondObjectId; })[0];
			...
			var firstObject = currentModel.objects.filter(function(item) { return item.id === arc.firstObjectId; })[0];
		...
		buttons: {
			'Ok': function() {
				$('.place-for-net').each(function() {
					...
					var arc = currentModel.arcs.filter(function(item) { return item.id === arcId; })[0];
	...
	$(document).on('contextmenu', function(e) {
		...
		if ($target.hasClass('petri-object')) {
			...
			var addMoreObjectsDialog = $('#addMoreObjectsPopup').dialog({
				...
				buttons: {
					'Cancel': function() { addMoreObjectsDialog.dialog('close'); },
					'Ok': function() {
