...
PetriNet.prototype.getClass = function() {
	return 'PetriNet';
};

PetriNet.prototype.getParameters = function() {
	var self = this;
...
PetriNet.prototype.hasParameters = function() {
	var self = this;
...
PetriNet.prototype.allDelaysAreZero = function() {
	var self = this;
...
PetriNet.prototype.isConnectedGraph = function() {
	var self = this;
...
PetriNet.prototype.validate = function() {
	var self = this;
	...
	for (var i = 0; i < placesCount; i++) {
		...
		var arcsNumber = place.arcs.filter(function(item) { return !item.isInformationLink; }).length;
		if (!arcsNumber) {
			return {
				valid: false,
	...
	for (var i = 0; i < transitionsCount; i++) {
		...
		var inputArcsNumber = transition.arcs.filter(function(item) { return item.fromPlace && !item.isInformationLink; }).length;
		var outputArcsNumber = transition.arcs.filter(function(item) { return !item.fromPlace; }).length;
...
PetriNet.prototype.draw = function() {
	var self = this;
