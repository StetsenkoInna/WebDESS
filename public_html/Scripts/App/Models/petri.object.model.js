...
PetriObjectModel.prototype.equalPlacesHaveEqualNumberOfMarkers = function() {
	...
	for (var i = 0; i < self.arcs.length; i++) {
		var arc = self.arcs[i];
		var firstObject = self.objects.filter(function(item) { return item.id === arc.firstObjectId; })[0];
		var secondObject = self.objects.filter(function(item) { return item.id === arc.secondObjectId; })[0];
		var firstPlace = firstObject.net.places.filter(function(place) { return place.id === arc.firstObjectPlaceId; })[0];
		var secondPlace = secondObject.net.places.filter(function(place) { return place.id === arc.secondObjectPlaceId; })[0]
...
PetriObjectModel.prototype.getEqualPlaceGroups = function() {
	...
	var groupContainsItem = function(group, item) {
		return group.filter(function(grItem) { return grItem.objectId === item.objectId && grItem.placeId === item.placeId; }).length > 0;
	};
	
	for (var i = 0; i < self.arcs.length; i++) {
		var arc = self.arcs[i];
		var newPlaces = [{ objectId: arc.firstObjectId, placeId: arc.firstObjectPlaceId }, { objectId: arc.secondObjectId, placeId: arc.secondObjectPlaceId }];
...
PetriObjectModel.prototype.generateJointNet = function() {
		...
		for (var k = 0; k < equalPlaceGroups.length; k++) {
			...
			if (groupContainsItem(group, item)) {
				for (var l = 0; l < group.length; l++) {
					...
					if (net.places.filter(function(pl) { return pl.oldId === groupItem.placeId && pl.objectId === groupItem.objectId; }).length > 0) {
						alreadyAdded = true;
						break;
					}
				}
	...
	var findAlreadyAddedPlace = function(placeId, objectId) {
		...
		for (var k = 0; k < equalPlaceGroups.length; k++) {
			...
			if (groupContainsItem(group, item)) {
				for (var l = 0; l < group.length; l++) {
					var groupItem = group[l];
					var placeResults = net.places.filter(function(pl) { return pl.oldId === groupItem.placeId && pl.objectId === groupItem.objectId; });
	...
	for (var i = 0; i < self.objects.length; i++) {
		...
		for (var j = 0; j < objNet.arcs.length; j++) {
			...
			arcCounter++;
			var arcPlace;
			var arcPlaces = net.places.filter(function(item) { return item.objectId === object.id && item.oldId === oldArc.placeId; });
			if (arcPlaces.length === 0) {
				arcPlace = findAlreadyAddedPlace(oldArc.placeId, object.id);
			} else {
				arcPlace = arcPlaces[0];
			}
			var arcTransition = net.transitions.filter(function(item) { return item.objectId === object.id && item.oldId === oldArc.transitionId; })[0];
