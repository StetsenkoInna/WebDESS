	...
	var dialog = $popup.dialog({
		...
		open: function() {
			$('#netInput').on('change', function() {
				...
				var newNet = restorePetriNet(parsePetriNet(netOptions.filter(function(item) { return item.netId === netId; })[0].net));
		...
		buttons: {
			...
			'Ok': function() {
				...
				self.netId = netId;
				self.net = restorePetriNet(parsePetriNet(netOptions.filter(function(item) { return item.netId === netId; })[0].net));
